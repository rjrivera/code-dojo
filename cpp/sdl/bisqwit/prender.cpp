...
#define EyeHeight 6 	// Camera height from floor standing
#define DuckHeight 2.5 	// Camera height ducking
#define HeadMargin 1 	// detection buffer head-ceiling
#define KneeHeight 2 	// tallest step height permitted
#define hfov (0.75f*H) 	// horizontal fov
#define vfov (0.2f*H) 	// vertical fov

using namespace std;

/* Sectors: Floor and ceiling height; list of edge vert. and neigh */

static struct sector //there can be ONLY ONE
{
	float floor, ceil;
	struct xy {float x, y; } *vertex;
	signed char *neighbors;
	unsigned npoints;	//num vertices in sector
...
#define vxp(x0,y0,x1,y1) ((x0)*(y1)) - ((x1)*(y0)) // vextor X-prod

//determine if two number ranges overlap.
#define Overlap(a0,a1,b0,b1) (min(a0,a1)<=max(b0,b1)&&min(b0,b1)<=max(a0,a1))

#define IntersectBox(x0,y0,x1,y1,x2,y2,x3,y3) (Overlap(x0,x1,x2,x3)&&Overlap(y0,y1,y2,y3)

#define PointSide(px,py,x0,y0,x1,y1) vxp((x1)-(x0),(y1)-(y0),(px)-(x0),(py)-(y0))

//#define Intersect(x1,y1,x2,y2,x3,y3,x4,y4) ((struct xy) {
//
#define Intersect(x1,y1, x2,y2, x3,y3, x4,y4) ((struct xy) { \
    vxp(vxp(x1,y1, x2,y2), (x1)-(x2), vxp(x3,y3, x4,y4), (x3)-(x4)) / vxp((x1)-(x2), (y1)-(y2), (x3)-(x4), (y3)-(y4)), \
    vxp(vxp(x1,y1, x2,y2), (y1)-(y2), vxp(x3,y3, x4,y4), (y3)-(y4)) / vxp((x1)-(x2), (y1)-(y2), (x3)-(x4), (y3)-(y4)) })
...
static void LoadData() {
	...
	while (fgets(Vuf, sizeof Buf, fp)) 
		switch(sscanf(ptr = Buf, "%32s%n", word, &n) == 1 ? word[0] : '\0') {
			case 'v': //vertex //this is a data parser
				for(sscanf(ptr +=n, "%f%n", &v.y, &n); sscanf(ptr += n, "%f%n", &v.x, &n) == 1; ) { 
					//ugh, realloc, c-code
					vert = realloc(vert, ++NumVertics * jsizeof(*vert)); vert[NumVertices-1] = v; }
					break;
			case 's': //secotr
